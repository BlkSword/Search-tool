...
lazy_static::lazy_static! {
    static ref SCAN_CACHE: ScanCache = ScanCache::new(100);
}
...
fn scan_directory_blocking(
    ...
    let mut all_files: Vec<(PathBuf, i64)> = Vec::new();
    ...
    all_files.par_iter().for_each(|(file_path, size)| {
        if let Some(parent) = file_path.parent() {
            for ancestor in parent.ancestors() {
                if ancestor == root_path.as_path() || ancestor == Path::new("") {
                    break;
                }
    ...
    let mut dir_sizes_map = HashMap::new();
    for (k, v) in dir_sizes.into_iter() {
        dir_sizes_map.insert(k.replace('\\', "/"), v);
    }

    let mut file_sizes = HashMap::new();
    for (file_path, size) in all_files {
        if let Some(path_str) = file_path.to_str() {
            file_sizes.insert(path_str.replace('\\', "/"), size);
        }
    }

    Ok((dir_sizes_map, file_sizes))
}

fn collect_files(path: &Path, files: &mut Vec<(PathBuf, i64)>) -> Result<(), anyhow::Error> {
